...
The hero is stripped to three things: **one line, the bevel, the action.** No sub-line, no app
icon, no hero download — the animation is the point. Download sits in the menu bar instead, so the
only above-fold conversion is that pill; the hero's button starts the demo. Worth testing.
...
as the "voice is a bonus" row in the practical bits. The close section now owns `id="download"`,
so the menu pill lands there.

**"copy to figma" really copies.** On fine-pointer devices the click builds an SVG of the current
...
label flips to "copied · now ⌘V in figma" for 2.6 s. `clipboard.writeText` can **throw
synchronously** (unfocused document), not just reject — both paths fall back to the `#download`
anchor, which is also the no-clipboard behaviour. Caveats: Figma substitutes the font unless the
paster has SF Pro; the SVG mirrors the board's geometry (680×580), not the app's exact export.
On **coarse pointers** the same button reads "send it to your mac" and opens a `mailto:` with the
page URL — no backend, honest about being a Mac app.
...
**"break it down" DRIVES the scroll.** One decisive **ease-out** glide (cubic, `GLIDE_MS` =
...
**Anchor jumps are coherent now, not dangerous.** `#how` scrolls past the zone, so the board
